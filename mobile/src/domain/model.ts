...
export const intimacyCount = (data: Data, id?: string) =>
  data.entries.filter(
    (e) => e.kind === "intimacy" && (!id || e.personId === id),
  ).length;
export const collection = (data: Data) =>
  data.people.filter((p) =>
    data.entries.some((e) => e.personId === p.id && e.kind === "intimacy"),
  );
export const personEntries = (data: Data, id: string) =>
  data.entries
...
export function upsertPerson(data: Data, person: Person): Data {
  ...
  if (
    person.scores.length !== 6 ||
    person.scores.some((n) => !Number.isFinite(n) || n < 0 || n > 10)
  )
    throw new Error("评分须在 0–10 之间");
  const clean = {
    ...person,
  ...
  return {
    ...data,
    people: [...data.people.filter((p) => p.id !== clean.id), clean],
  };
}
export function upsertEntry(data: Data, entry: Entry): Data {
  ...
  if (!validDate(entry.date))
    throw new Error("请填写有效日期，例如 2026-09-05");
  if (!entry.title.trim()) throw new Error("给这次相处起个标题吧");
  return {
    ...data,
    entries: [
      ...data.entries.filter((e) => e.id !== entry.id),
      { ...entry, title: entry.title.trim(), city: entry.city.trim() },
    ],
  };
}
export function removePerson(data: Data, id: string): Data {
  return {
    ...
    entries: data.entries.filter((e) => e.personId !== id),
  };
}
...
export function summary(data: Data, now = new Date()) {
  ...
  const months = Array.from({ length: 6 }, (_, i) => {
    ...
    return {
      ...
      count: data.entries.filter((e) => e.date.startsWith(key)).length,
    };
  });
  return {
    cities,
    months,
    collectionCount: collection(data).length,
    intimateCount: intimacyCount(data),
    average: data.people.length
      ? data.people.reduce((n, p) => n + scoreAverage(p.scores), 0) /
        data.people.length
      : 0,
  };
}
