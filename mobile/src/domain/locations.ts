...
export function findCity(name:string){return catalog.cities.find(c=>c.name.replace(/市$/,'')===name.trim().replace(/市$/,''));}
...
export function mapCoordinate(lat:number,lon:number){
  if(lon<72.004||lon>137.8347||lat<0.8293||lat>55.8271)return {latitude:lat,longitude:lon};
  ...
  const rad=lat/180*pi,sin=Math.sin(rad),magic=1-0.006693421622965943*sin*sin,sqrt=Math.sqrt(magic);
  dlat=dlat*180/((6378245*(1-0.006693421622965943))/(magic*sqrt)*pi);
