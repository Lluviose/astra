...
export default function Radar({ scores }: { scores: Scores }) {
  ...
  return <Svg width="100%" height={225} viewBox="0 0 240 225" accessible={false}>
    ...
    {dimensions.map((label, i) => { const p = point(i, 69), l = point(i, 93); return <React.Fragment key={label}><Line x1={120} y1={111} x2={p[0]} y2={p[1]} stroke="#E0E5DC" /><SvgText x={l[0]} y={l[1] + 4} fontSize="15" fill={c.muted} textAnchor="middle">{label}</SvgText></React.Fragment>; })}
