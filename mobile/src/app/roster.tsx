...
export default function Roster() {
  ...
  return <Screen back title="人物名册" right={<IconButton name="plus" label="新建人物" onPress={() => router.push("/person/edit")} />}>
    ...
    {people.length ? <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 14 }}>{people.map(p => <Pressable key={p.id} accessibilityRole="button" accessibilityLabel={`打开${hidden ? "人物" : p.name}档案`} onPress={() => router.push({ pathname: "/person/[id]", params: { id: p.id } })} style={{ width: width > 760 ? "31%" : "47%", marginBottom: 17 }}>
