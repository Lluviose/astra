...
- 新增和编辑记录、当场新建人物、可选细节、组合搜索、月份与人物筛选、跟进完成与撤销。记录与人物编辑共用底部保存区，错误就近显示，退出前保护未保存修改。
...
当前开发聚焦 iPhone。仓库三个云端工作流均仅手动触发，提交和 PR 更新不会自动运行。`React Native review` 手动运行时执行格式整理、类型与业务检查和 Expo 导出；只有明确勾选 `ios_simulator` 才会使用 macOS 云端运行器进行 iPhone 模拟器构建，该选项默认关闭。

保留的 XCTest 流程通过实际控件进入各页面，新增记录，并在应用重启后搜索确认持久化；手动启用原生检查时保存截图与日志。`public/review.html` 用固定手机宽度展示同一套 React Native Web 页面。本轮语法解析与 10 项业务测试通过，完整类型检查及新版原生截图验收尚未执行；逐页修改与审查边界见 `Docs/design-review.md`。
