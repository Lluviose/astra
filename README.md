...
- **猎艳概览**：首屏直接显示她的人数、累计约成、本月次数，以及「记一笔 / 加个人」两个入口
- **头像与艳照**：每人一张头像，每次约还能留最多 8 张照片；只复制进 App 沙盒，不进系统相册，也不进 iCloud 备份
- **成就**：约成、过夜、点亮城市、私藏照片等徽章，按现有记录现场计算，不做排行榜
- **对象档案**：代号 / 头像、相处状态（刚认识 → 聊上了 → 暧昧中 → 偶尔约 / 固定约 → 先搁着 / 结束了）、默契度、怎么约、她说过不行的事、安全备忘和标签
- **聊骚记录**：进展、她对黄段子 / 照片 / 视频的回应、聊过的尺度和规矩、线上隐私约定，以及转账、可疑链接等需要停手的信号
- **约成记录**：对象、时间和类型填了就能存；需要时再补做了什么、有没有戴套、爽不爽、还想不想约、照片和私密备注
- **事后跟进**：回个消息、再约一次、去做检测、暴露咨询等，可设日期并勾完成
- **安全小结**：戴套、PrEP、避孕分开记，不算「安全分」，也不按人打标签
- **联系节奏**：每个对象可设 7/14/30/60 天周期，到期只轻轻提醒；先搁着和结束了的人不会跳出来
- **城市足迹**：地图只标记到城市级；内置全国地级市、省直辖县级市、部分百强县和港澳台主要城市（380+），并处理 WGS-84 → GCJ-02 偏移
- **对象与回顾**：按状态 / 城市分组，支持代号、标签、城市搜索，以及近 6 个月约成次数
...
- **数据**：对象、记录和照片纯本地，不上传、不统计、不同步；支持 JSON 备份导出（含照片）/ 合并导入 / 一键清空
