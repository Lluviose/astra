...
- **结果记录**：原生三步表单「结果 → 细节 → 跟进」，两张结果大卡（细描边与勾选标明选择）、刚刚 / 昨晚 / 今早快捷时间、场所芯片，对象、结果、时间和地点确认好就能存；没上床可选进展和原因；上床了先记节奏（谁主动、几轮、多久），再按接吻与手上动作 / 口 / 性交 / 姿势 / 节奏 / 场景 / 情趣与玩法七组勾行为，收尾含潮吹，防护分开选；只复用上次行为与收尾，不自动填写这次防护、地点或花费
...
- **苹果风格界面**：系统蓝、浅色 / 石墨色内容卡片与每页一张的网格渐变封面（首页封面缓慢流动，减弱动态 / 低电量时静止），统一 SF Symbols 图标、留白和弹簧动效；滑动式胶囊选择器、弹簧进度条、成就勋章、Swift Charts 月趋势、图鉴到档案的缩放转场、下拉拉伸封面；导航、封面按钮和地图控件使用 iOS 26+ 原生 Liquid Glass，iOS 18 降级为 Material；适配减少透明度、增强对比度和减弱动态效果。设计说明与预览入口见 `Docs/apple-design.md`
