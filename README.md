...
- **战绩统计**：上床率、近 12 个月双柱趋势、套的分布、最常做的玩法、收尾与做完之后、什么氛围、我买单的比例与钱主要花在哪、清晨 / 白天 / 晚上 / 深夜与周几偏好、上得最多的人和地点、花费合计与平均、间隔与空窗、还想不想再约
...
- **时间线**：记录只分「上床了 / 没上床」，按天显示具体时间、地点和备注；可按代号、标签、地点、备注、玩法、事后、氛围、买单、跟进事项搜索；近 6 个月双柱对比，一键进完整统计；右滑一步勾完跟进或直达档案，长按可再记她一笔、打开档案、勾完跟进；删除前先确认，照片数量会一并提示
- **成就册**：60 枚徽章分成猎获、上床、复盘、足迹、私藏、玩法六章，铜银金三档，含她主动、持久战、三连发、潮吹、过夜、出境战绩、连续三月、摸清她等新章目，并聚合成从「猎场开张」到「全册封神」的 6 级王者身份
- **人物照、艳照与视频**：头像封面、普通人物资料照、艳照私藏和单次约会照片分开保存、分开浏览、分开计数；照片按原始字节保存、点开看原图（双指缩放、双击放大），视频原样保存（mov / mp4），相册和拍摄都能选视频，缩略图取首帧并显示时长，点开用系统播放器；档案相册是三列九宫格，超过 9 个可展开，长按删除；列表缩略图按需降采样并缓存，不改原图
- **经期**：每个人可单独记下开始 / 结束、流量和感觉；用近因加权稳健中位数估周期，丢掉过长空窗，最近几轮换档时跟新节奏走，排卵从下次经期回推较稳的黄体期；档案、首页和本机通知提醒快到、可能开始或推迟，锁屏默认不出现代号
- **猎获档案**：以她的头像色做封面，综合分环、「认识 X 天 · 第 Y 天上床」、上床 / 没上 / 私藏 / 上次上床四格加两个玻璃结果按钮；「见她之前看一眼」把她在床上喜欢什么、怎么约她最顺、说过的规矩和安全备忘放在最上面；经期预测和今天来了 / 走了；「和她的战绩」小结第一次上床、上床率、平均间隔、平均多久、平均几轮、谁更主动、最常做、套、花费、常约时段和最爽的一次；联系方式点一下复制；艳照与人物照分档切换；六维评分、基本线索和她的时间线
- **结果记录**：原生三步表单「结果 → 细节 → 跟进」，两张结果大卡（细描边与勾选标明选择）、刚刚 / 昨晚 / 今早快捷时间、场所芯片，对象、结果、时间和地点确认好就能存；没上床可选进展和原因；上床了先记节奏（谁主动、几轮、多久），再按接吻与手上动作 / 口 / 性交 / 姿势 / 节奏 / 场景 / 情趣与玩法七组勾行为，收尾含潮吹、她先到、一起到等 20 项，防护含 PEP、事后紧急避孕、近期检测；做完之后（抱着躺、一起洗、做完就走等 10 项，我走 / 她走 / 过夜互斥）、这次的氛围（激情 / 温柔 / 尴尬等 8 种，没上床也能记）、谁买单（我 / 她 / AA / 没花钱）与钱花在哪（房费、吃饭、打车、礼物等 8 项）分开记；只复用上次行为与收尾，不自动填写这次防护、地点或花费
...
- **事后跟进**：回个消息、再约一次、还她东西、发她照片、送个礼物、去做检测、暴露咨询等，可设日期；首页、时间线和档案里右滑或长按一步勾完成，不必打开表单
...
- **数据**：无账号、无统计、无应用内云同步；全部持久化用户数据允许进入设备 iCloud Backup，并支持明文 JSON 导出 / 合并导入（同一条记录以改得更晚的一份为准）/ 一键清空
...
| 权限 | 拍照 / 拍视频才要相机和麦克风；相册用系统选择器，不必交出整本相册。不读通讯录，不定位置。面容 / 触控 ID 可选。经期本机通知可选 |
| 照片 / 视频 | 头像、人物资料照、艳照和视频只存在 Application Support/AstraMedia，不写入系统相册，允许进入设备 iCloud Backup |
| 位置 | 中国只到**市**级，境外只到**国家**级；无精确坐标，无定位 |
| 防窥 | 可选 App 锁 + 后台毛玻璃遮罩 + 代号默认打码（打码时头像和相册也会糊掉） |
| 备份 | 档案、设置和全部照片、视频允许进入设备 iCloud Backup；另可导出含照片和视频的明文 JSON 自行保管 |

## 使用边界

星图只面向成年人之间自愿、知情且可随时撤回的相处。App 中的边界和安全字段用于记录已经明确沟通的信息，不代表一次记录可以替代下一次确认，也不替代专业健康建议。经期预测根据你记下的日期估算下次和窗口，不能替代医生，也不能当避孕依据。
...
App/Sources
  ...
  Domain/     Models · Achievement(含 RoyalRank) · Insights · PeriodCycle · City · ChinaRegion · CoordinateTransform · RosterFilter
  Data/       LocalStore · MediaStore · CityCatalog · BackupService · PeriodNotifications
